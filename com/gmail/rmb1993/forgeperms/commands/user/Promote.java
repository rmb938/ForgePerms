    ...
    public Promote(ICommandSender sender, String[] args) {
        if (args.length == 3) {
            ...
            if (ForgePermsAPI.playerHasPermission(sender.getCommandSenderName(), "permissions.promote.all")) {
                ...
                if (nextGroup == null) {
                    sender.sendChatToPlayer("Sorry "+args[1]+" can not be promoted anymore!");
                    return;
                }
                ...
                sender.sendChatToPlayer("You promoted " + args[1] + " to group " + nextGroup.getGroupName() + " in track " + args[2]);
                ...
                if (notInTrack == true) {
                    sender.sendChatToPlayer("You do not have permision to promote " + args[1] + " (Not in track " + args[2] + ")");
                    return;
                }
                Group groupInTrack = null;
                for (String gN: u1.getGroups()) {
                ...
                if (myGroup.getRank() > groupInTrack.getRank()) {
                    sender.sendChatToPlayer("You are not high enough rank to promote " + args[1]);
                    return;
                }
