        ...
        if (args.length == 3) {
            ...
            if (ForgePermsAPI.playerHasPermission(u.getUserName(), "permission.groupRank")) {
                ...
                try {
                    ...
                    sender.sendChatToPlayer(FontColour.RED + "Usage: /group setRank [groupName] [rank]");
                    return;
                }
                fpc.config.getDb().saveGroups();
                sender.sendChatToPlayer(FontColour.DARK_GREEN + "You set the rank of group " + args[1]
                        + " to " + args[2]);
            } else {
