    ...
    public RemoveGroupSuffix(ForgePermsContainer fpc, ICommandSender sender, String[] args) {
        if (args.length == 2) {
            ...
            if (ForgePermsAPI.playerHasPermission(u.getUserName(), "permission.groupSuffix")) {
                ...
                sender.sendChatToPlayer(FontColour.DARK_GREEN + "You removed the suffix from group " + args[1]);
            } else {
                sender.sendChatToPlayer(FontColour.RED + "You do not have permission to use this command.");
            }
        } else {
            sender.sendChatToPlayer(FontColour.RED + "Usage: /group removeSuffix [groupName]");
        }
    }
}
