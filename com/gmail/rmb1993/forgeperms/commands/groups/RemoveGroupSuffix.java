 ...
 *
 */
public class RemoveGroupSuffix {

    public RemoveGroupSuffix(ForgePermsContainer fpc, ICommandSender sender, String[] args) {
        if (args.length == 2) {
        	...
        	if (ForgePermsAPI.playerHasPermission(u.getUserName(), "permission.groupSuffix")) {
        		...
        		g.getVars().remove("suffix");
        		fpc.config.getDb().saveGroups();
        		sender.sendChatToPlayer(StringColors.EnumTextColor.DARK_GREEN.colorString("You removed the suffix from group " + args[1]));
        	} else {
        		sender.sendChatToPlayer(StringColors.EnumTextColor.RED.colorString("You do not have permission to use this command."));
        	}
        } else {
        	sender.sendChatToPlayer(StringColors.EnumTextColor.RED.colorString("Usage: /group removeSuffix [groupName]"));
        }
    }
