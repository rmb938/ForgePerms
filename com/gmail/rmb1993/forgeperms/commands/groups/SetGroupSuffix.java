    ...
    public SetGroupSuffix(ForgePermsContainer fpc, ICommandSender sender, String[] args) {
        if (args.length == 3) {
            ...
            if (ForgePermsAPI.playerHasPermission(u.getUserName(), "permission.groupSuffix")) {
                ...
                sender.sendChatToPlayer(FontColour.DARK_GREEN + "You set the suffix of group " + args[1]
                        + " to " + args[2]);
            } else {
                sender.sendChatToPlayer(FontColour.RED + "You do not have permission to use this command.");
            }
        } else {
            sender.sendChatToPlayer(FontColour.RED + "Usage: /group setSuffix [groupName] [suffix]");
        }
    }
}
